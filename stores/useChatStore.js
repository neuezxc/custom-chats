...
const useChatStore = create(
  persist(
    (set, get) => ({
      ...
      // Actions
      initializeConversations: () => set((state) => {
      ...
      updateCustomSystemPrompt: (settings) => {
        try {
          ...
          if (error.name === 'QuotaExceededError' || error.message.includes('quota')) {
            ...
            throw new Error(`Custom system prompt too large. ${errorResult.message}`)
          }
          throw error
        }
      },
      ...
      createCharacter: (characterData) => set((state) => {
        const newCharacter = {
          ...characterData,
      ...
      deleteCharacter: (characterId) => set((state) => {
        const { [characterId]: deleted, ...remainingCharacters } = state.characters
        return {
          characters: remainingCharacters,
          // Switch to default character if current was deleted
          currentCharacter: state.currentCharacter.id === characterId
            ? Object.values(remainingCharacters)[0] || state.currentCharacter
            : state.currentCharacter
        }
      }),
      ...
      updateApiSettings: (updates) => set((state) => ({
        apiSettings: { ...state.apiSettings, ...updates }
      })),

      // Lorebook Management Actions
      addLorebookEntry: (characterId, entry) => set((state) => {
        if (!state.characters[characterId]) return state
      ...
      updateLorebookEntry: (characterId, entryId, updates) => set((state) => {
        if (!state.characters[characterId]) return state
      ...
      deleteLorebookEntry: (characterId, entryId) => set((state) => {
        if (!state.characters[characterId]) return state

        const character = state.characters[characterId]
        const updatedLorebook = character.lorebook.filter(entry => entry.id !== entryId)
      ...
      setCharacterGalleryMode: (characterId, mode) => set((state) => {
        if (!state.characters[characterId]) return state
      ...
      addCharacterImage: (characterId, imageData) => set((state) => {
        if (!state.characters[characterId]) return state
        ...
        if (imageGallery.images.length >= maxImages) {
          console.warn(`Cannot add more images. Maximum ${maxImages} images allowed for ${imageGallery.mode} mode.`)
          return state
        }
        ...
        const updatedCharacter = {
          ...
          imageGallery: {
            ...
            activeImageIndex: imageGallery.images.length === 0 ? 0 : imageGallery.activeImageIndex
          }
        }
      ...
      removeCharacterImage: (characterId, imageId) => set((state) => {
        if (!state.characters[characterId]) return state

        const character = state.characters[characterId]
        const { imageGallery } = character

        const imageIndex = imageGallery.images.findIndex(img => img.id === imageId)
        ...
        const updatedImages = imageGallery.images.filter(img => img.id !== imageId)
        let newActiveIndex = imageGallery.activeImageIndex

        // Adjust active index if necessary
        if (imageIndex <= imageGallery.activeImageIndex && updatedImages.length > 0) {
      ...
      setActiveCharacterImage: (characterId, index) => set((state) => {
        if (!state.characters[characterId]) return state
      ...
      updateLastLLMResponse: (response) => set({
        lastLLMResponse: response
      }),

      setCharacterEmotion: (characterId, emotion) => set((state) => ({
        emotionStates: {
          ...state.emotionStates,
          [characterId]: {
      ...
      clearEmotionState: (characterId) => set((state) => {
        const { [characterId]: removed, ...remainingStates } = state.emotionStates
        return {
      ...
      validateImageUpload: (file, currentMode, existingImages) => {
        ...
        if (!allowedTypes.includes(file.type)) {
          validationErrors.push('Please upload JPG, PNG, GIF, or WebP images only')
        }
        ...
        if (currentMode === 'multiple') {
          const totalSize = existingImages.reduce((sum, img) => sum + (img.size || 0), 0) + file.size
      ...
      generateImagePreview: (file, maxWidth = 800, maxHeight = 600, quality = 0.8) => {
        return new Promise((resolve, reject) => {
          ...
          reader.onload = (e) => {
            ...
            img.onload = () => {
              ...
              console.log(`Image compression: ${originalSize} -> ${compressedSize} (${(compressionRatio * 100).toFixed(1)}%)`)
          ...
          characters: Object.fromEntries(
            Object.entries(state.characters).map(([id, char]) => [
              id,
              {
                ...
                imageGallery: {
                  ...char.imageGallery,
                  images: char.imageGallery?.images?.filter(img => !img.isUrl) || []
                }
              }
            ])
          )
      ...
      cleanupLargestImages: () => set((state) => {
        ...
        Object.keys(updatedCharacters).forEach(charId => {
          ...
          if (char.imageGallery?.images?.length > 0) {
            ...
            const sortedImages = [...char.imageGallery.images].sort((a, b) => b.size - a.size)
            const imagesToKeep = sortedImages.slice(Math.floor(sortedImages.length / 2)) // Keep smallest 50%

            // Clean up blob URLs for removed images
            sortedImages.slice(0, Math.floor(sortedImages.length / 2)).forEach(img => {
            ...
            updatedCharacters[charId] = {
              ...
              imageGallery: {
                ...char.imageGallery,
                images: imagesToKeep,
                activeImageIndex: Math.min(char.imageGallery.activeImageIndex, imagesToKeep.length - 1)
              }
            }
          }
        })

        console.log(`Cleaned up ${totalCleaned} images to free storage space`)
        return {
          ...state,
      ...
      handleStorageQuotaError: (error) => {
        ...
        if (usage.mb > 4) { // If over 4MB, try cleanup
          ...
          return {
            ...
            suggestion: 'Consider using images under 1MB or enabling compression.'
          }
        }

        return {
          success: false,
          message: 'Storage limit exceeded. Try removing some characters or images to free up space.',
          suggestion: 'Use smaller images or enable image compression to save space.'
        }
      },
      ...
      deleteUserMessage: (messageIndex) => set((state) => {
        ...
        const { startIndex, endIndex } = get().calculateDeletionRange(messages, messageIndex)
        ...
        deletedMessages.forEach(msg => {
          if (msg.versions && msg.versions.length > 1) {
            console.log(`Deleting message ${msg.id} with ${msg.versions.length} versions`)
          }
        })

        // Check for welcome message in deletion range
        const welcomeMessage = get().findWelcomeMessage(messages, startIndex, endIndex, currentCharacter)
      ...
      sendMessage: async (text) => {
        const { addMessage, setIsTyping, currentCharacter, apiSettings, validateMessage } = get()
        ...
        try {
          ...
          const triggeredLore = detectTriggeredLorebook(validatedText, currentCharacter)
          ...
          for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
              if (apiSettings.provider === 'gemini') {
                response = await callGeminiAPI(validatedText, currentCharacter, apiSettings, conversationContext, triggeredLore)
              } else {
                response = await callOpenRouterAPI(validatedText, currentCharacter, apiSettings, conversationContext, triggeredLore)
              }
      ...
      regenerateLastMessage: async () => {
        const { messages, isTyping, currentCharacter, apiSettings, setIsTyping, addMessage } = get()
        ...
        if (lastMessage.type !== 'character' || lastMessage.isError) {
          console.warn('Cannot regenerate: last message is not a valid character message')
          return
        }
        ...
        try {
          ...
          const conversationContext = prepareConversationContext(updatedMessages, 10)

          // Detect triggered lorebook entries from the original user message
          const triggeredLore = detectTriggeredLorebook(userMessage.content, currentCharacter)
          ...
          for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
              if (apiSettings.provider === 'gemini') {
                response = await callGeminiAPI(userMessage.content, currentCharacter, apiSettings, conversationContext, triggeredLore)
              } else {
                response = await callOpenRouterAPI(userMessage.content, currentCharacter, apiSettings, conversationContext, triggeredLore)
              }
              break // Success, exit retry loop
            } catch (error) {
              lastError = error
      ...
      regenerateMessageWithVersions: async (messageId) => {
        const { messages, currentCharacter, apiSettings, isTyping, isRegenerating } = get()

        // Enhanced validation checks
        if (isTyping || isRegenerating) {
          console.warn('Cannot regenerate while typing or already regenerating')
          return
        }
        ...
        if (!hasApiKey) {
          ...
          if (messageIndex !== -1) {
            ...
            const errorVersion = get().createMessageVersion('Please configure your API key in settings.', true)
            const updatedMessage = get().addVersionToMessage(message, errorVersion)
        ...
        const parentUserMessage = get().findParentUserMessage(messages, messageIndex)
        ...
        try {
          ...
          regenerationTimeout = setTimeout(() => {
            ...
            const updatedMessage = get().addVersionToMessage(message, timeoutErrorVersion)
          ...
          const conversationContext = prepareConversationContext(contextMessages, 10)

          // Detect triggered lorebook entries
          const triggeredLore = detectTriggeredLorebook(parentUserMessage.content, currentCharacter)
          ...
          for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
              if (apiSettings.provider === 'gemini') {
                response = await callGeminiAPI(parentUserMessage.content, currentCharacter, apiSettings, conversationContext, triggeredLore)
              } else {
                response = await callOpenRouterAPI(parentUserMessage.content, currentCharacter, apiSettings, conversationContext, triggeredLore)
              }
              break // Success, exit retry loop
            } catch (error) {
              lastError = error
          ...
          console.log('Error categorization:', { errorMessage, errorType, isRetryable })
      ...
      changeMessageVersion: (messageId, direction) => {
        set((state) => {
          ...
          if (newIndex > message.versions.length) newIndex = message.versions.length
      ...
      enableMessageEdit: (messageId) => set((state) => {
      ...
      updateMessage: (messageId, newContent) => set((state) => {
        const messageIndex = state.messages.findIndex(msg => msg.id === messageId)
        if (messageIndex === -1) return state
      ...
      editLastUserMessage: async (newContent) => {
        const { messages, isTyping, validateMessage, currentCharacter, apiSettings } = get()
        ...
        try {
          ...
          // Detect triggered lorebook entries
          const triggeredLore = detectTriggeredLorebook(validatedContent, currentCharacter)
          ...
          for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
              if (apiSettings.provider === 'gemini') {
                response = await callGeminiAPI(validatedContent, currentCharacter, apiSettings, conversationContext, triggeredLore)
              } else {
                response = await callOpenRouterAPI(validatedContent, currentCharacter, apiSettings, conversationContext, triggeredLore)
              }
              break // Success, exit retry loop
            } catch (error) {
              lastError = error
          ...
          // Categorize errors for better user feedback
          let errorMessage = 'Sorry, I encountered an error while regenerating the response.'
          let errorType = 'generic'
      ...
      cancelMessageEdit: (messageId) => set((state) => ({
        messages: state.messages.map(msg =>
          msg.id === messageId
            ? { ...msg, isEditing: false }
            : msg
...
async function callGeminiAPI(userMessage, character, apiSettings, conversationContext = [], triggeredLore = []) {
  const { customSystemPrompt } = useChatStore.getState()
  const systemPrompt = buildSystemPrompt(character, triggeredLore, customSystemPrompt)
  ...
  try {
    ...
    if (!lastMessage || lastMessage.role !== 'user' || !lastMessage.parts[0].text.includes(userMessage)) {
      contents.push({
        role: 'user',
    ...
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${apiSettings.selectedModel}:generateContent?key=${apiSettings.geminiApiKey}`, {
      method: 'POST',
      headers: {
...
async function callOpenRouterAPI(userMessage, character, apiSettings, conversationContext = [], triggeredLore = []) {
  const { customSystemPrompt } = useChatStore.getState()
  const systemPrompt = buildSystemPrompt(character, triggeredLore, customSystemPrompt)
